...
```sh
make docker-build docker-push IMG=<some-registry>/rancher-config-sync-operator:tag
```
...
```sh
make deploy IMG=<some-registry>/rancher-config-sync-operator:tag
```
...
```sh
make build-installer IMG=<some-registry>/rancher-config-sync-operator:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/rancher-config-sync-operator/<tag or branch>/dist/install.yaml
```
